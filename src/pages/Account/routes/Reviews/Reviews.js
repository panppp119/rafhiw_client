...
class Reviews extends React.Component {
  ...
  render() {
    ...
    reviews.map((review) => (avg += review.get('rating')))
      ...
      <div className='reviews'>
        ...
        <div className='body'>
          <div className='rating'>
            ...
            <div className='star'>
              <FaStar
                className={ClassNames({ active: avg >= 1 && avg <= 5 })}
              />
              <FaStar
                className={ClassNames({ active: avg >= 2 && avg <= 5 })}
              />
              <FaStar
                className={ClassNames({ active: avg >= 3 && avg <= 5 })}
              />
              <FaStar
                className={ClassNames({ active: avg >= 4 && avg <= 5 })}
              />
          ...
          <div className='comments'>
            {!reviews.isEmpty() ? (
              reviews.map((review, i) => {
                ...
                const rating = review.get('rating') || 0
                ...
                return (
                  <div className='comment' key={i}>
                    ...
                    <div className='content'>
                      ...
                      <p className='product'>
                        ({product.get('name')} - {product.get('po_name')})
                      </p>
                      <div
                        className='star'
                      ...
                      >
                        <FaStar
                          className={ClassNames({
                            active: rating >= 1 && rating <= 5,
                          })}
                        />
                        <FaStar
                          className={ClassNames({
                            active: rating >= 2 && rating <= 5,
                          })}
                        />
                        <FaStar
                          className={ClassNames({
                            active: rating >= 3 && rating <= 5,
                          })}
                        />
                        <FaStar
                          className={ClassNames({
                            active: rating >= 4 && rating <= 5,
                          })}
                        />
                        <FaStar className={ClassNames({ active: avg === 5 })} />
                      </div>
